...
void update_result(){
   ...
   string type = last_accepted_state->get_state_type();
      ...
      for(int i=0; i<word.size(); i++){
          ...
          if(nfa_next_states.size()==0){
              if(last_accepted_index==i-1 || !reach_accepted_state){
                   set_error("Accepted state not found");
                   return;
              }
              update_result();
       ...
       if(!reach_accepted_state || last_accepted_index!=(word.size()-1)) set_error("Word End without Accepted state");
       update_result();
}
